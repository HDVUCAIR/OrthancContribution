...
# ----------------------------------------------------------------------------
    ...
    if pg_connection is None and pg_cursor is None:
        ...
        if pg_connection is None or pg_cursor is None:
            return {'status':1, 'error_text': 'load_phi_to_anon_map: Must provide both con and cur'}, None
...
# ----------------------------------------------------------------------------
    ...
    if pg_cursor.rowcount == 0:
        if flag_local_db:
            pg_cursor.close()
            pg_connection.close()
        orthanc.LogWarning('Tried to insert patientname anon into nonexistent siuid2patientname_anon')
...
# ----------------------------------------------------------------------------
    ...
    try:
        ...
        if flag_local_db:
            pg_cursor.close()
            pg_connection.close()
        return {'status': 2, 'error_text' : 'on_orthanc:Problem querying siuid'}, None, None, None
    row = pg_cursor.fetchone()
    n_rows = 0
    sql_siuid = None
    while row is not None:
    ...
    if sql_siuid is not None:
        ...
        try:
            ...
            if flag_local_db:
                pg_cursor.close()
                pg_connection.close()
            return {'status': 5, 'error_text' : 'on_orthanc:Problem querying anon siuid'}, None, None, None
        row = pg_cursor.fetchone()
        n_rows = 0
        while row is not None:
...
# -------------------------------------------------------
    ...
    orthanc.LogWarning('Checking whether remote user (%s) is permitted to act on %s' % (remote_user,uri))
...
# ----------------------------------------------------------------------------
    ...
    if method == 'POST' and \
        ...
        if study_res is not None:
            ...
            #if not flag_js:
            #    orthanc.LogWarning('Starting old style anon')
            #    return True
    ...
    if user_permitted(uri, get_remote_user(request['headers'])):
        try:
            ...
            if flag_current == 'true':
                orthanc.LogWarning('LUA_FLAG_ASSUME_ORIGINAL_PRIMARY is true, turning false...')
                ...
                orthanc.LogWarning('LUA_FLAG_ASSUME_ORIGINAL_PRIMARY is false, turning true...')
            ...
            orthanc.LogWarning('Problem getting LUA_FLAG_ASSUME_ORIGINAL_PRIMARY state')
